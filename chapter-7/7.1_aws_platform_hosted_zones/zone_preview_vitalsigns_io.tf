...
module "subdomain_preview_vitalsigns_io" {
  ...
  zones = {
    "preview.${local.domain_vitalsigns_io}" = {
      tags = {
  ...
  records = [
    {
      ...
      records         = module.subdomain_preview_vitalsigns_io.route53_zone_name_servers["preview.${local.domain_vitalsigns_io}"]
    }
  ]

  depends_on = [module.subdomain_preview_vitalsigns_io]
}
