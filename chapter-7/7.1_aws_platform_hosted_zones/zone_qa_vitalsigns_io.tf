...
provider "aws" {
  alias  = "subdomain_qa_vitalsigns_io"
  region = "us-east-2"
  assume_role {
...
module "subdomain_qa_vitalsigns_io" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "3.1.0"
  create  = true

  providers = {
    aws = aws.subdomain_qa_vitalsigns_io
  }

  zones = {
    "qa.${local.domain_vitalsigns_io}" = {
      tags = {
...
module "subdomain_zone_delegation_qa_vitalsigns_io" {
  ...
  records = [
    {
      ...
      records         = module.subdomain_qa_vitalsigns_io.route53_zone_name_servers["qa.${local.domain_vitalsigns_io}"]
    }
  ]

  depends_on = [module.subdomain_qa_vitalsigns_io]
}
