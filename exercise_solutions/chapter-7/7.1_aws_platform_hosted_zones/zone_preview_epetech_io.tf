...
provider "aws" {
  alias  = "subdomain_preview_epetech_io"
  region = "us-east-1"
  assume_role {
...
module "subdomain_preview_epetech_io" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "5.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_preview_epetech_io
  }

  zones = {
    "preview.${local.domain_epetech_io}" = {
      tags = {
...
module "subdomain_zone_delegation_preview_epetech_io" {
  ...
  records = [
    {
      ...
      records         = module.subdomain_preview_epetech_io.route53_zone_name_servers["preview.${local.domain_epetech_io}"]
    }
  ]

  depends_on = [module.subdomain_preview_epetech_io]
}
