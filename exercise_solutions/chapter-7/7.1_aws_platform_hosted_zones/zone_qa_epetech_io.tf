...
module "subdomain_qa_epetech_io" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "5.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_qa_epetech_io
  }

  zones = {
    "qa.${local.domain_epetech_io}" = {
      tags = {
...
module "subdomain_zone_delegation_qa_epetech_io" {
  ...
  records = [
    {
      ...
      records         = module.subdomain_qa_epetech_io.route53_zone_name_servers["qa.${local.domain_epetech_io}"]
    }
  ]

  depends_on = [module.subdomain_qa_epetech_io]
}
