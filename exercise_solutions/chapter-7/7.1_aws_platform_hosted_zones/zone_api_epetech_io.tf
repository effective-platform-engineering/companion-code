...
provider "aws" {
  alias  = "subdomain_api_epetech_io"
  region = "us-east-2"
  assume_role {
...
module "subdomain_api_epetech_io" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "5.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_api_epetech_io
  }

  zones = {
    "api.${local.domain_epetech_io}" = {
      tags = {
...
module "subdomain_zone_delegation_api_epetech_io" {
  ...
  records = [
    {
      ...
      records         = module.subdomain_api_epetech_io.route53_zone_name_servers["api.${local.domain_epetech_io}"]
    }
  ]

  depends_on = [module.subdomain_api_epetech_io]
}
